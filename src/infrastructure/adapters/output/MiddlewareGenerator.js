...
class MiddlewareGenerator extends BaseFileGenerator {
  ...
  async generate(basePath) {
    ...
    const templateFiles = (await this.fileService.readDir(this.templateDir))
      .filter(file => file.endsWith(".ejs"));

    for (const file of templateFiles) {
      const templatePath = this.fileService.resolvePath(this.templateDir, file);
      const rendered = await this.renderTemplate(templatePath);
      const outputName = file.replace(/\.ejs$/, ".js");

      const writtenPath = await this.writeRenderedFile(outputPath, outputName, rendered);
